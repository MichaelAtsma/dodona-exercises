...
def evaluate_test_type_check(context, display_expected_on_fail, expected_int_value, feedback_shown=True):
    ...
    correct_value = int(context.actual) == expected_int_value

    if not correct_value:
        mymessages.append(Message(f"De waarde is niet wat we verwachtten."))

    same_value = context.actual == expected_int_value

    if same_value:
        mymessages.append(Message(f"Je hebt de waarde {repr(expected_int_value)} gebruikt. Je mag deze waarde niet exact gebruiken. Bekijk nog eens in het voorbeeld hoe floats worden omgezet naar integers."))
    
    correct = correct_type and correct_value and not same_value
