...
if not bulk_test:
    ...
    while True:
        ...
        n -= 1
    X = edge_cases.copy()
    non_primes = GenerateSequentialNonPrimes(1000000, min_factors=2, max_factors=2, skip_step=0)
    primes = GenerateSequentialPrimes(amount//2 + 1, skip_first=5, skip_step=11)
    while len(X) < amount:
        if len(X) % 2 == 0:
            non_prime = next(non_primes)
            non_square = random.random() < 0.5
            while non_square and ProductDelersValue(non_prime) == non_prime**2:
                print(f"{len(X) = }. Skipping {non_prime} because the product of its divisors is its square.")
                non_prime = next(non_primes)
            X.append((non_prime,))
        else:
