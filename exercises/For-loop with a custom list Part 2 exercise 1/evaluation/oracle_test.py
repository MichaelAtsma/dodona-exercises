...
def evaluate_test(context, match_regex, list_variable_name, mandatory_texts_and_descriptions, forbidden_texts_and_descriptions, correct_message_template, wrong_value_message_template):
    ...
    checks["student contribution type does not contain strings"] = '"' not in student_contribution and "'" not in student_contribution
    ...
    if correct:
        ...
        if not checks["list variable name used correctly"]:
            mymessages.append(Message(f"Zorg ervoor dat je in de for-lus de juiste naam voor de lijst gebruikt."))
        if not checks["student contribution type does not contain strings"]:
            mymessages.append(Message(f"Hoewel het antwoord misschien juist is, moet je getallen printen. Dan heb je dus geen aanhalingstekens nodig."))
