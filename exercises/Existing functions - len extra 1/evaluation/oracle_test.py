...
def evaluate_test_type_check(context, display_expected_on_fail):
    ...
    correct_length = len(str(context.actual)) == len(str(context.expected))

    if not correct_length:
        mymessages.append(Message(f"De waarde is niet wat we verwachtten."))
    
    correct = correct_type and correct_length
...
def evaluate_test(context, result_var, ingredient_vars, mandatory_symbols_and_display_names, forbidden_symbols):
    ...
    mymessages = []
    if correct:
        mymessages.append(Message(f"Goed zo! Je hebt de computer {repr(context.expected)} laten berekenen op basis van {' en '.join(ingredient_vars)} in de len functie."))
    else:
